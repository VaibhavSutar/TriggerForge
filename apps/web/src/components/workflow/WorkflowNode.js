...
import { Settings, Play, Clock, Mail, Webhook, Database, Code, Filter, Edit2, Check, X } from 'lucide-react';
...
export const WorkflowNode = memo(({ id, data, selected }) => {
    ...
    const renderConfigInput = (key, value) => {
        ...
        <input type={inputType} value={value || ''} onChange={(e) => handleConfigUpdate(key, inputType === 'number' ? Number(e.target.value) : e.target.value)} className="w-full px-2 py-1 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500" placeholder={`Enter ${key}`}/>
      </div>);
    };
    ...
    `}>
      <Handle type="target" position={Position.Top} className="w-3 h-3 transition-all duration-200 hover:scale-125" style={{ background: '#6366f1' }}/>
      ...
      <div className="flex items-center space-x-3 mb-2">
        ...
        <div className="flex-1 min-w-0">
          {isEditingName ? (<div className="flex items-center space-x-1">
              <input type="text" value={tempName} onChange={(e) => setTempName(e.target.value)} className="flex-1 text-sm font-medium bg-transparent border-b border-blue-500 focus:outline-none" onKeyDown={(e) => {
                if (e.key === 'Enter')
                    handleNameSave();
                if (e.key === 'Escape')
                    handleNameCancel();
            ...
            </div>) : (<div className="flex items-center space-x-1 cursor-pointer group" onClick={() => setIsEditingName(true)}>
        ...
        <button onClick={() => setIsConfigOpen(!isConfigOpen)} className={`
      ...
      <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-100">
        ...
        {Object.keys(tempConfig).length > 0 && (<span className="text-xs text-gray-400">
            {Object.keys(tempConfig).length} config{Object.keys(tempConfig).length !== 1 ? 's' : ''}
          </span>)}
      </div>

      <Handle type="source" position={Position.Bottom} className="w-3 h-3 transition-all duration-200 hover:scale-125" style={{ background: '#6366f1' }}/>
    </div>);
});
