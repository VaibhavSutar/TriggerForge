...
export const WORKFLOW_TEMPLATES: WorkflowTemplate[] = [
    ...
    },
    {
        ...
        nodes: [
            ...
            { id: "4", connectorId: "ai", label: "Draft Custom Email", position: { x: 100, y: 550 }, config: { model: "gemini-2.0-flash", prompt: "Write a short, engaging 3-sentence outreach email to {{ input[0] }} who is interested in {{ input[2] }}. Be friendly." } },
    ...
    },
    {
        ...
        nodes: [
            ...
            { id: "2", connectorId: "ai", label: "Write Newsletter", position: { x: 100, y: 250 }, config: { model: "gemini-2.0-flash", prompt: "Write today's edition of my tech newsletter. Cover 3 major recent advancements in tech. Keep it engaging, friendly, and structured. (Do not include placeholder names)." } },
    ...
    },
    {
        ...
        nodes: [
            ...
            { id: "3", connectorId: "ai", label: "Extract Lead Info (JSON)", position: { x: 100, y: 400 }, config: { model: "gemini-2.0-flash", prompt: "Extract lead information from the following search results (provided in JSON): {{ $node.2.output.organic_results }}{{ $node.2.output.leads_json }}. Focus on finding companies that might need our services. Return ONLY a JSON array of objects. Keys: 'companyName', 'contactPerson', 'email', 'website', 'industry'. Example: [{\"companyName\": \"Tech Corp\", \"contactPerson\": \"John Doe\", \"email\": \"john@techcorp.com\", \"website\": \"techcorp.com\", \"industry\": \"Software\"}]. If you cannot find a contact person or email, make an educated guess based on the website snippet or use a generic one like 'info@company.com'." } },
    ...
    },
    {
        ...
        nodes: [
            ...
            { id: "3", connectorId: "ai", label: "Extract Leads (JSON)", position: { x: 100, y: 400 }, config: { model: "gemini-2.0-flash", prompt: "Extract business leads from these search results: {{ $node.2.output.organic_results }}{{ $node.2.output.leads_json }}. Return a JSON array with 'name', 'website', and 'niche'." } },
            ...
            { id: "7", connectorId: "ai", label: "Generate Outreach Email", position: { x: 100, y: 1000 }, config: { model: "gemini-2.0-flash", prompt: "Write a short custom outreach email for {{ input[0] }} in the {{ input[2] }} niche." } },
    ...
    },
    {
        ...
        nodes: [
            ...
            { id: "4", connectorId: "ai", label: "Analyze Context (JSON)", position: { x: 100, y: 550 }, config: { model: "gemini-2.0-flash", prompt: "Analyze this email: Subject: '{{ input.subject }}' Body: '{{ input.body }}'. Return a JSON object with two fields: 'isSpam' (boolean) and 'tags' (array of strings like 'Invoice', 'Newsletter', 'Urgent')." } },
